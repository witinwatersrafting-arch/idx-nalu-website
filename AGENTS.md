...
2. **全自動圖片處理協定 (Project-Internal Auto-Image Protocol)**：
   ...
   - **步驟 A：命名決策**：根據草稿主題生成一個 SEO 友善的英文檔名（如 `kayaking-tutorial`）。
   - **步驟 B：執行轉檔**：在終端機執行 `bash scripts/auto-image.sh "你生成的檔名"`。
   - **步驟 C：讀取清單**：精確獲取終端機輸出的 `.webp` 檔名清單。
   - **步驟 D：寫入 HTML**：將圖片插入 HTML，路徑一律使用 `images/檔名.webp`。
   
   **【絕對禁令】**：
   - 嚴禁自行改名或搬移檔案，必須統一調用 `auto-image.sh`。
   - 若腳本回傳找不到圖片，請提醒使用者將圖片丟入桌面捷徑或專案內的 `rawimage/`。
